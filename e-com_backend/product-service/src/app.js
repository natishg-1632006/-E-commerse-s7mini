...
app.use(compression());
// Product CORS config
const prodAllowed = ['https://d2c24kno5aj17g.cloudfront.net', 'https://d222r50ryi3b71.cloudfront.net', 'http://localhost:3000', 'http://localhost:5173'];
app.use(
  cors({
    origin: (origin, cb) => {
      const isCloudfront = origin && origin.endsWith('.cloudfront.net');
      const allowed = !origin || isCloudfront || prodAllowed.includes(origin) || (process.env.ALLOWED_ORIGINS && process.env.ALLOWED_ORIGINS.split(',').includes(origin));
      cb(null, allowed);
    },
    credentials: true,
